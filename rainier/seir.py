...
#### Mortality related optimization functions
...
def DeathRSS(theta,model,beta_t,pulse_indices,observed_deaths,
			 mean_ifr=0.01,mean_delay=19):
...
#### Hospitalization related optimization functions
def MeanHosp(model,beta_t,z_t,sig_eps,
			 mean_ihr=0.03,mean_delay=6):
	X = model.mean(beta_t,z_t,sig_eps)
	mean_exposures = X[1,1:] - (1. - (1./model.D_e))*X[1,:-1]
	destined_hosp = mean_ihr*mean_exposures
	mean_hosp = np.hstack([np.zeros((mean_delay+1,)),destined_hosp[:-mean_delay]])
	return mean_hosp
